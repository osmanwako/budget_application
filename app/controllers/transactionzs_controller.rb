class TransactionzsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @transactionz.save
        format.html { redirect_to new_category_transactionz_path(@category), notice: "Transaction was successfully created." }
      else
  ...
  def new
    @category = Category.find_by(id: params[:category_id])
    str = "This transaction is for healthcare. Enrollment and disenrollment. Premium payment. Referral certification and authorization. "
    if @category.nil?
      redirect_to categories_path, alert: "Category not found."
    end
  ...
  def transactionz_params
    params.require(:transactionz).permit(:name, :amount, :description, :logo, :category_id).merge(author_id: current_user.id)
  end
end
